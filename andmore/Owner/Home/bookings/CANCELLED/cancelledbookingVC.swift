...
class cancelledbookingVC: UITableViewController {
        ...
        if let venueIDz = selectedShop?.venueID {
            self.venueID = venueIDz
               ...
               func fetchDesks() {
                  ...
                  let ref = Database.database().reference().child("VenueAppointments").child("cancel").child(venueID)
  ...
  //                        self.fetchuser(id: key)
                 ...
                 override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
                         ...
                         let cell = cancelTableView.dequeueReusableCell(withIdentifier: "Cell", for: indexPath) as! cancelledBookingCell
             ...
             performSegue(withIdentifier: "toCancel", sender: selectedShop)
                 ...
                 override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
                             if segue.identifier == "toCancel" {
                               
                               
                         if let selectedPhone = sender as? boodkingDashboardSystem,
                             let destinationVC = segue.destination as? UINavigationController {
                               if let childVc = destinationVC.topViewController as? detailedCancelledVC {

...
class HourlistVC: UITableViewController {
    ...
    func welcoming() {
        ...
        Database.database().reference().child("All").child(uid).observeSingleEvent(of: .value) { (snapshot) in
            ...
            self.fetchHourScheulde(string:self.type)
        }
    }
           
    func fetchHourScheulde(string:String) {
           guard let uid = Auth.auth().currentUser?.uid else {return}
                  
        let ref = Database.database().reference().child("stores").child("Allbuisness").child(string).child(uid).child("mySchedule").child("Hour")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            let cell = HourTableView.dequeueReusableCell(withIdentifier: "Cell", for: indexPath) as! hourlistCell
    ...
    //
            ...
            performSegue(withIdentifier: "Tob", sender: selectedShop)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
           ...
           if let selectedPhone = sender as? scheduleModel,
               let destinationVC = segue.destination as? hourAvailabilityVC {
